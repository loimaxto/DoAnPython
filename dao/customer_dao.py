import sys
import os
# Get the absolute path to the 'project' directory (parent of 'da' and 'db')
...
class KhachHangDAO:
    def __init__(self, db_filename="db/hotel7-3.db"): #Use the default db filename from the SQLiteDB class.
        self.db = SQLiteDB(db_filename)
    ...
    def insert_khach_hang(self, khach_hang):
        ...
        result = self.db.execute_query(query, (khach_hang.ten, khach_hang.sdt, khach_hang.image))
        if result is not None:
            query_last_id = "SELECT last_insert_rowid()"
            last_id = self.db.execute_query(query_last_id)
            if last_id:
                return last_id[0][0] #return the last inserted ID.
        return None
...
if __name__ == "__main__":
    ...
    # Example insert
    new_customer = KhachHangDTO(ten="John Doe", sdt="123-456-7890", image="john.jpg")
